...
This repository now provides two conda environment files at the project root:

* `environment.yml`: GPU training/inference (PyTorch 1.12.1 + CUDA 11.3)
...
conda activate mvsc-py39

# CPU
conda env create -f environment.cpu.yml
conda activate mvsc-py39-cpu
```
...
### MVSC training script

A dedicated MVSC training entry is provided in `train_mvsc.py`.

Example:

```bash
python train_mvsc.py \
  --train-root E:/your_scene/exo \
  --val-root E:/your_scene_val/exo \
  --output-dir ./runs/mvsc_exp1 \
  --epochs 30 \
  --batch-size 2 \
  --device cuda \
  --channel-type awgn \
...
Multi-sequence example (auto-discover all `*/exo`):

```bash
python train_mvsc.py \
  --train-root E:/Harmony4D/train/02_grappling \
  --val-root E:/Harmony4D/train/01_hugging \
  --output-dir ./runs/mvsc_grappling \
  --epochs 30 \
  --batch-size 2 \
  --device cuda \
  --channel-type awgn \
...
* Input dataset should follow the Harmony4D exo folder structure used by `data/harmony4d_mvsc.py`.
* `--train-root` / `--val-root` can be either one `exo` folder or a parent directory containing multiple `*/exo` folders. The loader will discover and merge them automatically.
* By default, when multiple `exo` roots are discovered, `--train-repeat` / `--val-repeat` are split as a total budget across all roots to keep epoch length stable. Use `--repeat-per-exo` if you want repeat to apply to each root.
* Current channel implementation (`awgn` / `rayleigh`) expects CUDA tensors. On CPU, use `--channel-type none`.
