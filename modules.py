...
class PatchMerging4x(nn.Module):
    def __init__(self, input_resolution, dim, out_dim=None, norm_layer=nn.LayerNorm):
        ...
        if out_dim is None:
            out_dim = dim
        self.patch_merging1 = PatchMerging((H, W), dim, out_dim=out_dim, norm_layer=norm_layer)
        self.patch_merging2 = PatchMerging((H // 2, W // 2), out_dim, out_dim=out_dim, norm_layer=norm_layer)
...
class PatchReverseMerging4x(nn.Module):
    def __init__(self, input_resolution, dim, out_dim, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        H, W = input_resolution
        self.patch_reverse_merging1 = PatchReverseMerging((H, W), dim, out_dim=dim, norm_layer=norm_layer)
        self.patch_reverse_merging2 = PatchReverseMerging((H * 2, W * 2), dim, out_dim=out_dim, norm_layer=norm_layer)
    ...
    def flops(self):
        H, W = self.input_resolution
        flops = 0
        flops += self.patch_reverse_merging1.flops()
        flops += self.patch_reverse_merging2.flops()
        return flops
...
def window_reverse_3d(windows, window_size, D, H, W):
    ...
    Args:
        windows: (num_windows*B, Wd, Wh, Ww, C)
        window_size: (Wd, Wh, Ww)
        D, H, W: output spatial-temporal resolution
    Returns:
        x: (B, D, H, W, C)
    """
    Wd, Wh, Ww = window_size
...
class WindowAttention3D(nn.Module):
    """
    Simplified 3D window attention.
    Input: (num_windows*B, N, C), where N = Wd*Wh*Ww
    """
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        ...
        size = (2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] - 1)
...
class PatchMerging3D(nn.Module):
    """
    3D patch merging over spatial dimensions only.
    Input: x of shape [B, D, H*W, C]
    Output: [B, D, (H/2)*(W/2), out_dim]
    """
    def __init__(self, input_resolution, dim, out_dim=None, norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, x):
        B, D, L, C = x.shape
        Td, H, W = self.input_resolution
        assert D == Td, f"Temporal size mismatch: {D} vs {Td}"
        assert L == H * W, f"Token size mismatch: {L} vs {H * W}"
        assert H % 2 == 0 and W % 2 == 0, f"x size ({H}*{W}) are not even."
        ...
        x = x.view(B, D, (H // 2) * (W // 2), 4 * C)
        x = self.norm(x)
        x = self.reduction(x)
        return x

class PatchReverseMerging3D(nn.Module):
    """
    3D reverse patch merging over spatial dimensions only.
    Input: [B, D, H*W, C]
    Output: [B, D, (2H)*(2W), out_dim]
    """
    def __init__(self, input_resolution, dim, out_dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution  # (D, H, W)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = self.increment(x)
        x = x.view(B * D, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = nn.PixelShuffle(2)(x)
        x = x.flatten(2).permute(0, 2, 1).contiguous()
        x = x.view(B, D, (H * 2) * (W * 2), self.out_dim)
        return x