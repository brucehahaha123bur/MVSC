...
class Channel(nn.Module):
    ...
    def complex_normalize(self, x, power):
        # Use float32 statistics to avoid fp16 underflow in AMP mode.
        x_safe = torch.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)
        pwr = torch.mean(x_safe.float() ** 2) * 2.0
        pwr = torch.clamp(pwr, min=1e-12)
        scale = float(np.sqrt(power)) / torch.sqrt(pwr)
        out = x_safe * scale.to(dtype=x.dtype)
        return out, pwr
    ...
    def forward(self, input, chan_param, avg_pwr=False):
        ...
        tx_dtype = channel_tx.dtype
        channel_in = channel_tx.reshape(-1)
        # ComplexHalf is still experimental in PyTorch; use float32 for complex channel ops.
        if channel_in.dtype in (torch.float16, torch.bfloat16):
            channel_in = channel_in.float()
        ...
        channel_output = channel_output.reshape(input_shape).to(dtype=tx_dtype)
...
def forward_mvsc(self, x, snr_db, avg_pwr=None):
    return self.forward(x, snr_db, avg_pwr)