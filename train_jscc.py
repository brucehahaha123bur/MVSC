...
# NOTE:
# This trainer uses the validated encoder/decoder pipeline from `encoder1.py`
# and `decoder1.py`, together with the multi-scene Harmony4D dataset loader.
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="runs/mvsc_jscc", help="Directory for logs/checkpoints")
    ...
    parser.add_argument(
        ...
        type=str,
        default="jscc_only",
        choices=["jscc_only", "full_finetune", "scratch_full"],
        help=(
            "JSCC training phase. jscc_only freezes individual/commonality and only trains JSCC; "
            "full_finetune loads a checkpoint and finetunes end-to-end; "
            "scratch_full trains the full pipeline from scratch."
        ),
    )
    parser.add_argument(
        "--init-ckpt",
        type=str,
        default="",
        help="Optional checkpoint path used for weights-only initialization before training.",
    )
    parser.add_argument(
        "--init-strict",
        action="store_true",
        help="Load --init-ckpt with strict=True. Default uses strict=False.",
    )
...
class ModernMVSCNet(nn.Module):
    def __init__(self, args):
        ...
        # Current commonality encoder compresses only time T, not views V.
        # Therefore JSCC decoder must restore with the same compressed view count Vc = num_views.
        ...
        # decoder starts from 320 then goes back to 256, so use reversed tuples.
...
def main():
    ...
    # Phase presets for JSCC training.
    if args.phase == "jscc_only":
        args.freeze_individual = True
        args.freeze_commonality = True
    elif args.phase == "full_finetune":
        args.freeze_individual = False
        args.freeze_commonality = False
    elif args.phase == "scratch_full":
        args.freeze_individual = False
        args.freeze_commonality = False
    else:
        raise ValueError(f"Unsupported phase: {args.phase}")

    if args.phase in {"jscc_only", "full_finetune"} and str(args.init_ckpt).strip() == "":
        raise ValueError(f"--init-ckpt is required for phase={args.phase}")
    ...
    if str(args.init_ckpt).strip() != "":
        print(f"[Info] Initialize model weights from: {args.init_ckpt}")
        ...
        if isinstance(ckpt, dict) and "model" in ckpt:
            ...
            raise TypeError(f"Unsupported checkpoint format: {args.init_ckpt}")
        ...
        for key, value in raw_state_dict.items():
            if args.phase == "jscc_only" and (key.startswith("jscc_encoder.") or key.startswith("jscc_decoder.")):
                skipped_phase.append(key)
                continue
        ...
        print(f"[Info] init-ckpt loaded tensors: {len(filtered_state)}")
        if skipped_phase:
            print(f"[Info] init-ckpt skipped JSCC tensors for phase={args.phase}: {len(skipped_phase)}")
        if skipped_missing:
            print(f"[Warn] init-ckpt unknown keys: {len(skipped_missing)}")
        if skipped_shape:
            print(f"[Warn] init-ckpt shape-mismatch keys: {len(skipped_shape)}")
            for key, src_shape, dst_shape in skipped_shape[:10]:
                print(f"[Warn]   shape mismatch: {key} ckpt={src_shape} current={dst_shape}")
            if len(skipped_shape) > 10:
                print(f"[Warn]   ... and {len(skipped_shape) - 10} more shape-mismatch keys")

        missing = list(getattr(incompatible, "missing_keys", []))
        unexpected = list(getattr(incompatible, "unexpected_keys", []))
        if missing:
            print(f"[Warn] init-ckpt missing keys after filtered load: {len(missing)}")
        if unexpected:
            print(f"[Warn] init-ckpt unexpected keys after filtered load: {len(unexpected)}")
    ...
    if len(trainable_names) > 20:
        print(f"[Info]   ... and {len(trainable_names) - 20} more")
    ...
    if str(args.init_ckpt).strip() == "":
        ...
        print("[Info] Training from initialized weights.")
    ...
    print(
        f"[Info] Phase: {args.phase}, train_stage={model.train_stage}, "
        f"freeze_individual={args.freeze_individual}, freeze_commonality={args.freeze_commonality}, "
        f"channel_type={args.channel_type}"
    )
    print(f"[Info] init_ckpt={'none' if str(args.init_ckpt).strip() == '' else args.init_ckpt}")
